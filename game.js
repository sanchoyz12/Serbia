const storyData = {
    ...
    legal_status_ok: {
        ...
        choices: [
            { text: "Идти штурмовать банки.", next: "bank_quest", effect: { status: "Легальный турист" } },
            { text: "Сначала найти постоянное жилье.", next: "apartment_hunt", effect: { status: "Легальный турист" } }
        ]
    },
    bank_quest: {
    ...
    bank_success: {
        ...
        choices: [
            { text: "Да! Ищу ближайшую кафану.", next: "kafana_night", effect: { money: -30, stress: -40, status: "Клиент банка" } },
            { text: "Нет, нужно срочно искать жилье.", next: "apartment_hunt", effect: { status: "Клиент банка" } }
        ]
    },
    kafana_night: {
    ...
    job_offer: {
        ...
        choices: [
            { text: "Подаваться на боравак через работу.", next: "boravak_process_local", effect: { status: "Будущий резидент" } }
        ]
    },
    boravak_process_local: {
        ...
        choices: [
            { text: "Забрать ВНЖ.", next: "ending_local_legend", effect: { status: "Резидент" } }
        ]
    },
    viza_ran_bosnia: {
    ...
    ending_local_legend: {
        ...
        choices: [{ text: "Начать новую жизнь", next: "start", effect: { status: "Местный" } }]
    },
    ending_business_mogul: {
        ...
        choices: [{ text: "Начать новую жизнь", next: "start", effect: { status: "Магнат" } }]
    },
    ending_eternal_nomad: {
        ...
        choices: [{ text: "Начать новую жизнь", next: "start", effect: { status: "Номад" } }]
    },
    ending_montenegro_escape: {
        ...
        choices: [{ text: "Начать новую жизнь", next: "start", effect: { status: "Отдыхающий" } }]
    },
    ending_back_home: {
        ...
        choices: [{ text: "Начать новую жизнь", next: "start", effect: { status: "Москвич" } }]
    }
};
...
class ShaderEffects {
    ...
    draw() {
        ...
        // 1. Optimized Film Grain Effect
        // Instead of generating pixels per frame, draw a noise pattern and shift it
        this.ctx.fillStyle = `rgba(${Math.random() * 50}, ${Math.random() * 50}, ${Math.random() * 50}, 0.05)`;
        for (let i = 0; i < 50; i++) {
...
class Game {
    ...
    handleChoice(choice) {
        if (choice.effect) {
            ...
            if (choice.effect.status) this.status = choice.effect.status;
        }
